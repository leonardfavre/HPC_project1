...
#define PAD 4 // for 32 byte L1 cache line size
#include <xmmintrin.h>
        ...
        *isamp = *osamp = len;
        ...
        while (len--) {
                ...
                *obuf++ = out * 256;
                ...
                //optimisation : remplacer le modulo par une logique moins gourmande :
                //remplace : chorus->counter = ( chorus->counter + 1 ) % chorus->maxsamples;
                chorus->counter += 1;
                if(chorus->counter = chorus->maxsamples)
                	chorus->counter = 0;
                //fin du remplacement
                for ( i = 0; i < chorus->num_chorus; i++ ){
                	//remplace : chorus->phase[i]  = ( chorus->phase[i] + 1 ) % chorus->length[i];
                	chorus->phase[i] += 1;
